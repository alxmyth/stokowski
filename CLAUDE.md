...
- **Docker mode: plugin config files are rewritten for containers**: Claude Code discovers plugins primarily through `known_marketplaces.json` (`installLocation` fields), with `installed_plugins.json` as secondary metadata. Both files store absolute host paths and are typically mode `0600`. `_prepare_plugin_file()` in `docker_runner.py` reads each file on the host, rewrites paths to container equivalents, writes a `0644` temp copy, and bind-mounts it read-only over the original. In DooD mode (orchestrator itself in a container), the host files aren't accessible — the fallback mounts a tmpfs at `~/.claude/plugins` (shadowing the bind mount for that subtree only) and a read-only secondary mount of the host's plugins dir at `/host-claude-plugins`. The fixup script copies from the read-only mount into the tmpfs, rewrites paths there, and fixes `marketplaces/` directory permissions. This prevents writes from reaching the host filesystem while preserving session persistence on the parent bind mount.
